...
function Report() {
  ...
  useEffect(() => {
    ...
    console.log(`Executing logic for report: ${reportName}`);

    setLoading(false);
    setLoading2(false);
  }, [reportName]);

  async function fetchReportData(selectedValue) {
    
    setLoading(true);
    setLoading2(true);
    const response = await axios.get(`/report/${selectedValue}`);
    
    const data = response.data;console.log(data.datas)
...
const formattedDates = xvalues.map((timestamp) => {
  ...
  const options = {
    ...
    title: {
      text: `${reportName.slice(0, -7).toUpperCase()} REPORT Time Chart`,
    },
    xAxis: {
